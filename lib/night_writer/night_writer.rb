...
class NightWriter
  ...
  def parse_string(input_string)
    ...
    parsed_array = input_array.map { |char| parse_character(char) }
    parsed_string = parsed_array.join.split('').join
  end
  ...
  #   output_string = ''
  #   insert_new_line if input_string.length > 40
  ...
  def build_third_of_line(input_string, third_position)
    ...
    input_string.each_char do |char|
      output += alphabet[char][third_position]
    end
    output
  end
